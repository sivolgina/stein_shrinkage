...
class ModifiedJSBatchNorm(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, running_mean, running_var, weight, bias, training, momentum, eps):
        ...
        N = n * h * w *d
        M = 4*(c-1)*(1 - 1/(N+1))/((N-1)*c+2)
        sigma_prod = torch.prod(sigmab)
        sigma_prod = sigma_prod.clamp(min=1e-6)
        sigmajs = (1 - 1/(N+1))* sigmab + M*sigma_prod ** (1.0 / c)

        invstd = (sigmajs + eps).rsqrt_()
        xhat = (x - mujs) * invstd
        y = xhat * weight + bias
        ctx.save_for_backward(x, mub, sigmab, mumub, sigmamub, sigma_prod, mujs, sigmajs, invstd, xhat, weight, eps)
        return y
    
    @staticmethod
    @torch.autograd.function.once_differentiable
    def backward(ctx, dl_y):
        x, mub, sigmab, mumub, sigmamub, sigma_prod, mujs, sigmajs, invstd, xhat, gamma, eps = ctx.saved_tensors
        n, c, h, w,d = x.shape
        N = n * h * w * d
        M = 4*(c-1)*N/(N+1)/((N-1)*c+2)
        ...
        dsummub_mub = 2 * mub
        dsigma_prod_sigmab = sigma_prod/(sigmab + 1e-6)
        dmujs_sigmamub = - mub * (c - 2)*c/(c+1) / (mub ** 2).sum()
        dmub_x = 1 / N
        dsigmab_x = 2 * (x - mub) / N
        dsigmamub_mub = 2 * (mub - mumub) / c

        dl_sigmajs = (dl_xhat * dxhat_sigmajs).sum(dim=[0, 2, 3,4]).reshape(1, c, 1, 1,1)
...
class BatchNorm3d(torch.nn.BatchNorm3d):
    def forward(self, x):
        return ModifiedJSBatchNorm.apply(x, self.running_mean, self.running_var, self.weight, self.bias, self.training, self.momentum, self.eps)


def convert_batchnorm_stein(module):
    ...
    for name, child in module.named_children():
        module_output.add_module(name, convert_batchnorm_stein(child))
    del module
